...
class BMP3XX:
    ...
    def __init__(self, debug=False):
        ...
        self.sea_level_pressure = BMP3XX.STANDARD_SEA_LEVEL_PRESSURE_PA
        self._debug = debug
    ...
    def begin(self):
        ...
        """
        ret = True
        chip_id = self._read_reg(BMP3XX.BMP3XX_CHIP_ID, 1)
        self.dprint("chip id: %d" % chip_id[0])
        if chip_id[0] not in (BMP3XX.BMP388_CHIP_ID, BMP3XX.BMP390L_CHIP_ID):
            ret = False
    ...
    def calibrated_absolute_difference(self, altitude):
        ...
        ret = False
        if BMP3XX.STANDARD_SEA_LEVEL_PRESSURE_PA == self.sea_level_pressure:
            self.sea_level_pressure = (self.pressure / pow(1.0 - (altitude / 44307.7), 5.255302))
            ret = True
        return ret
    ...
    def altitude(self):
        ...
        return 44307.7 * (1 - (self.pressure / BMP3XX.STANDARD_SEA_LEVEL_PRESSURE_PA) ** 0.190284)
    ...
    def set_power_mode(self, mode):
        ...
        """
        temp = self._read_reg(BMP3XX.BMP3XX_PWR_CTRL, 1)[0]
        if (mode | 0x03) == temp:
            self.dprint("Same configuration as before!")
        else:
            if mode != BMP3XX.SLEEP_MODE:
                self._write_reg(BMP3XX.BMP3XX_PWR_CTRL, (BMP3XX.SLEEP_MODE & 0x30) | 0x03)
                utime.sleep_ms(20)
            self._write_reg(BMP3XX.BMP3XX_PWR_CTRL, (mode & 0x30) | 0x03)
            utime.sleep_ms(20)
    ...
    def enable_fifo(self, mode):
        ...
        if mode:
            ...
            self._write_reg(BMP3XX.BMP3XX_FIFO_CONF_1, 0x1D)
            self._write_reg(BMP3XX.BMP3XX_FIFO_CONF_2, 0x0C)
        else:
            # 关闭FIFO。
            self._write_reg(BMP3XX.BMP3XX_FIFO_CONF_1, 0x1C)
            self._write_reg(BMP3XX.BMP3XX_FIFO_CONF_2, 0x0C)
        utime.sleep_ms(20)
    ...
    def set_oversampling(self, press_osr_set, temp_osr_set):
        ...
        """
        self._write_reg(BMP3XX.BMP3XX_OSR, (press_osr_set | temp_osr_set) & 0x3F)
    ...
    def filter_coefficient(self, iir_config_coef):
        ...
        self._write_reg(BMP3XX.BMP3XX_IIR_CONFIG, iir_config_coef & 0x0E)
    ...
    def set_output_data_rates(self, odr_set):
        ...
        # The IIR filter coefficient.
        ret = True
        self._write_reg(BMP3XX.BMP3XX_ODR, odr_set & 0x1F)
        if (self._read_reg(BMP3XX.BMP3XX_ERR_REG, 1)[0] & 0x04):
            self.dprint("Sensor configuration error detected!")
            ret = False
        return ret
    ...
    def set_common_sampling_mode(self, mode):
        ...
        """
        ret = True
        if mode == BMP3XX.ULTRA_LOW_PRECISION:
            self.set_power_mode(BMP3XX.FORCED_MODE)
            self.set_oversampling(BMP3XX.BMP3XX_PRESS_OSR_SETTINGS[0], BMP3XX.BMP3XX_TEMP_OSR_SETTINGS[0])
            self.filter_coefficient(BMP3XX.BMP3XX_IIR_CONFIG_COEF_0)
            self.set_output_data_rates(BMP3XX.BMP3XX_ODR_0P01_HZ)
        elif mode == BMP3XX.LOW_PRECISION:
            self.set_power_mode(BMP3XX.NORMAL_MODE)
            self.set_oversampling(BMP3XX.BMP3XX_PRESS_OSR_SETTINGS[1], BMP3XX.BMP3XX_TEMP_OSR_SETTINGS[0])
            self.filter_coefficient(BMP3XX.BMP3XX_IIR_CONFIG_COEF_0)
            self.set_output_data_rates(BMP3XX.BMP3XX_ODR_100_HZ)
        elif mode == BMP3XX.NORMAL_PRECISION1:
            self.set_power_mode(BMP3XX.NORMAL_MODE)
            self.set_oversampling(BMP3XX.BMP3XX_PRESS_OSR_SETTINGS[2], BMP3XX.BMP3XX_TEMP_OSR_SETTINGS[0])
            self.filter_coefficient(BMP3XX.BMP3XX_IIR_CONFIG_COEF_3)
            self.set_output_data_rates(BMP3XX.BMP3XX_ODR_50_HZ)
        elif mode == BMP3XX.NORMAL_PRECISION2:
            self.set_power_mode(BMP3XX.NORMAL_MODE)
            self.set_oversampling(BMP3XX.BMP3XX_PRESS_OSR_SETTINGS[3], BMP3XX.BMP3XX_TEMP_OSR_SETTINGS[0])
            self.filter_coefficient(BMP3XX.BMP3XX_IIR_CONFIG_COEF_1)
            self.set_output_data_rates(BMP3XX.BMP3XX_ODR_50_HZ)
        elif mode == BMP3XX.HIGH_PRECISION:
            self.set_power_mode(BMP3XX.NORMAL_MODE)
            self.set_oversampling(BMP3XX.BMP3XX_PRESS_OSR_SETTINGS[3], BMP3XX.BMP3XX_TEMP_OSR_SETTINGS[0])
            self.filter_coefficient(BMP3XX.BMP3XX_IIR_CONFIG_COEF_1)
            self.set_output_data_rates(BMP3XX.BMP3XX_ODR_12P5_HZ)
        elif mode == BMP3XX.ULTRA_PRECISION:
            self.set_power_mode(BMP3XX.NORMAL_MODE)
            self.set_oversampling(BMP3XX.BMP3XX_PRESS_OSR_SETTINGS[4], BMP3XX.BMP3XX_TEMP_OSR_SETTINGS[1])
            self.filter_coefficient(BMP3XX.BMP3XX_IIR_CONFIG_COEF_3)
            self.set_output_data_rates(BMP3XX.BMP3XX_ODR_25_HZ)
        else:
            ret = False
        return ret
    ...
    def enable_data_ready_int(self):
        ...
        """
        self._write_reg(BMP3XX.BMP3XX_INT_CTRL, 0x42)
        utime.sleep_ms(20)
    ...
    def enable_fifo_wtm_int(self, wtm_value):
        ...
        """
        self._write_reg(BMP3XX.BMP3XX_INT_CTRL, 0x0A)
        self._write_reg(BMP3XX.BMP3XX_FIFO_WTM_0, wtm_value & 0xFF)
        self._write_reg(BMP3XX.BMP3XX_FIFO_WTM_1, (wtm_value >> 8) & 0x01)
        utime.sleep_ms(20)
    ...
    def enable_fifo_full_int(self):
        """!
          ...
          @n    separately, please note the other two interrupt functions when using
        """
        self._write_reg(BMP3XX.BMP3XX_INT_CTRL, 0x12)
        utime.sleep_ms(20)
    ...
    def _get_coefficients(self):
        ...
        """
        calib = self._read_reg(BMP3XX.BMP3XX_CALIB_DATA, 21)
        self._data_calib = (
    ...
    def _get_reg_temp_press_data(self):
        ...
        """
        data = self._read_reg(BMP3XX.BMP3XX_P_DATA_PA, 6)
    ...
    def get_fifo_temp_press_data(self):
        ...
        """
        data = self._read_reg(BMP3XX.BMP3XX_FIFO_DATA, BMP3XX.BMP3XX_FIFO_DATA_FRAME_LENGTH)
        adc_p, adc_t = 0, 0
        if data[0] == BMP3XX.BMP3_FIFO_TEMP_PRESS_FRAME:
            adc_t = data[3] << 16 | data[2] << 8 | data[1]
            adc_p = data[6] << 16 | data[5] << 8 | data[4]
        elif data[0] == BMP3XX.BMP3_FIFO_TEMP_FRAME:
            adc_t = data[3] << 16 | data[2] << 8 | data[1]
        elif data[0] == BMP3XX.BMP3_FIFO_PRESS_FRAME:
            adc_p = data[3] << 16 | data[2] << 8 | data[1]
        elif data[0] == BMP3XX.BMP3_FIFO_TIME_FRAME:
            self.dprint("FIFO time: ", (data[3] << 16 | data[2] << 8 | data[1]))
        elif data[0] == BMP3XX.BMP3_FIFO_CONFIG_CHANGE:
            self.dprint("FIFO config change!!!")
        else:
            self.dprint("FIFO ERROR!!!")
    ...
    def get_fifo_length(self):
        ...
        """
        len = self._read_reg(BMP3XX.BMP3XX_FIFO_LENGTH, 2)
    ...
    def empty_fifo(self):
        ...
        """
        self._write_reg(BMP3XX.BMP3XX_CMD, BMP3XX.BMP3XX_CMD_FIFO_FLUSH)
        utime.sleep_ms(20)
    ...
    def reset(self):
        ...
        """
        self._write_reg(BMP3XX.BMP3XX_CMD, BMP3XX.BMP3XX_CMD_RESET)
...
class BMP3XX_I2C(BMP3XX):
    ...
    def __init__(self, i2c, i2c_addr=0x77, debug=False):
        """!
